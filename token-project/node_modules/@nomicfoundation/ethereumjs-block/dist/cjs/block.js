...
class Block {
    ...
    static async genWithdrawalsTrieRoot(wts, emptyTrie) {
        ...
        for (const [i, wt] of wts.entries()) {
            await trie.put(ethereumjs_rlp_1.RLP.encode(i), ethereumjs_rlp_1.RLP.encode(wt.raw()));
        }
        return trie.root();
    }
    /**
    ...
    static fromRLPSerializedBlock(serialized, opts) {
        const values = ethereumjs_rlp_1.RLP.decode(Uint8Array.from(serialized));
    ...
    static fromValuesArray(values, opts) {
        ...
        if (header.common.isActivatedEIP(6800) && executionWitnessBytes !== undefined) {
            executionWitness = JSON.parse((0, ethereumjs_util_1.bytesToUtf8)(ethereumjs_rlp_1.RLP.decode(executionWitnessBytes)));
        }
        else {
    ...
    static async fromExecutionPayload(payload, opts) {
        ...
        for (const [index, serializedTx] of transactions.entries()) {
            try {
                const tx = ethereumjs_tx_1.TransactionFactory.fromSerializedData((0, ethereumjs_util_1.hexToBytes)(serializedTx), {
                    common: opts?.common,
                });
                txs.push(tx);
            }
        ...
        const transactionsTrie = await Block.genTransactionsTrieRoot(txs, new ethereumjs_trie_1.Trie({ common: opts?.common }));
        const withdrawals = withdrawalsData?.map((wData) => ethereumjs_util_1.Withdrawal.fromWithdrawalData(wData));
        const withdrawalsRoot = withdrawals
            ? await Block.genWithdrawalsTrieRoot(withdrawals, new ethereumjs_trie_1.Trie({ common: opts?.common }))
            : undefined;
        const header = {
            ...payload,
            number,
        ...
        if (!(0, ethereumjs_util_1.equalsBytes)(block.hash(), (0, ethereumjs_util_1.hexToBytes)(payload.blockHash))) {
            const validationError = `Invalid blockHash, expected: ${payload.blockHash}, received: ${(0, ethereumjs_util_1.bytesToHex)(block.hash())}`;
    ...
    raw() {
        ...
        if (this.executionWitness !== undefined && this.executionWitness !== null) {
            const executionWitnessBytes = ethereumjs_rlp_1.RLP.encode(JSON.stringify(this.executionWitness));
     ...
     */
    async genTxTrie() {
        return Block.genTransactionsTrieRoot(this.transactions, new ethereumjs_trie_1.Trie({ common: this.common }));
    }
    /**
    ...
    async transactionsTrieIsValid() {
        ...
        if (this.transactions.length === 0) {
            result = (0, ethereumjs_util_1.equalsBytes)(this.header.transactionsTrie, ethereumjs_util_1.KECCAK256_RLP);
            return result;
        }
        if (this.cache.txTrieRoot === undefined) {
            this.cache.txTrieRoot = await this.genTxTrie();
        }
        result = (0, ethereumjs_util_1.equalsBytes)(this.cache.txTrieRoot, this.header.transactionsTrie);
        return result;
    }
    /**
    ...
    getTransactionsValidationErrors() {
        const errors = [];
        let blobGasUsed = ethereumjs_util_1.BIGINT_0;
    ...
    uncleHashIsValid() {
        ...
        const raw = ethereumjs_rlp_1.RLP.encode(uncles);
        return (0, ethereumjs_util_1.equalsBytes)(this.keccakFunction(raw), this.header.uncleHash);
    }
    /**
    ...
    async withdrawalsTrieIsValid() {
        ...
        const withdrawalsRoot = await Block.genWithdrawalsTrieRoot(this.withdrawals, new ethereumjs_trie_1.Trie({ common: this.common }));
        return (0, ethereumjs_util_1.equalsBytes)(withdrawalsRoot, this.header.withdrawalsRoot);
    }
    /**
    ...
    validateUncles() {
        ...
        // Header does not count an uncle twice.
        const uncleHashes = this.uncleHeaders.map((header) => (0, ethereumjs_util_1.bytesToHex)(header.hash()));
...
Block.fromJsonRpcProvider = async (provider, blockTag, opts) => {
    let blockData;
    const providerUrl = (0, ethereumjs_util_1.getProvider)(provider);
    if (typeof blockTag === 'string' && blockTag.length === 66) {
        blockData = await (0, ethereumjs_util_1.fetchFromProvider)(providerUrl, {
        ...
        blockData = await (0, ethereumjs_util_1.fetchFromProvider)(providerUrl, {
            ...
            params: [(0, ethereumjs_util_1.bigIntToHex)(blockTag), true],
        });
    }
    ...
    if (blockData.uncles.length > 0) {
        for (let x = 0; x < blockData.uncles.length; x++) {
            const headerData = await (0, ethereumjs_util_1.fetchFromProvider)(providerUrl, {
                method: 'eth_getUncleByBlockHashAndIndex',
                params: [blockData.hash, (0, ethereumjs_util_1.intToHex)(x)],
            });
