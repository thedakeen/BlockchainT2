...
async function genGenesisStateRoot(genesisState, common) {
    ...
    if (genCommon.isActivatedEIP(6800)) {
        ...
        return (0, ethereumjs_trie_1.genesisStateRoot)(genesisState);
    }
}
/**
...
async function getGenesisStateRoot(chainId, common) {
    const chainGenesis = ethereumjs_common_1.ChainGenesis[chainId];
...
class Blockchain {
    ...
    constructor(opts = {}) {
        ...
        this.db = opts.db !== undefined ? opts.db : new ethereumjs_util_1.MapDB();
    ...
    async _init(opts = {}) {
        ...
        if (stateRoot === undefined) {
            if (this._customGenesisState !== undefined) {
                ...
                stateRoot = await getGenesisStateRoot(Number(this.common.chainId()), this.common);
            }
        }
        const genesisBlock = opts.genesisBlock ?? this.createGenesisBlock(stateRoot);
        let genesisHash = await this.dbManager.numberToHash(ethereumjs_util_1.BIGINT_0);
        const dbGenesisBlock = genesisHash !== undefined ? await this.dbManager.getBlock(genesisHash) : undefined;
        ...
        if (dbGenesisBlock !== undefined && !(0, ethereumjs_util_1.equalsBytes)(genesisBlock.hash(), dbGenesisBlock.hash())) {
        ...
        if (!dbGenesisBlock) {
            ...
            const dbOps = [];
            dbOps.push((0, helpers_js_1.DBSetTD)(genesisBlock.header.difficulty, ethereumjs_util_1.BIGINT_0, genesisHash));
            (0, helpers_js_1.DBSetBlockOrHeader)(genesisBlock).map((op) => dbOps.push(op));
            (0, helpers_js_1.DBSaveLookups)(genesisHash, ethereumjs_util_1.BIGINT_0).map((op) => dbOps.push(op));
    ...
    async resetCanonicalHead(canonicalHead) {
        await this.runWithLock(async () => {
            ...
            const dbOps = [];
            await this._deleteCanonicalChainReferences(canonicalHead + ethereumjs_util_1.BIGINT_1, hash, dbOps);
        ...
        await this.runWithLock(async () => {
            ...
            try {
                const block = item instanceof ethereumjs_block_1.BlockHeader
                    ? new ethereumjs_block_1.Block(item, undefined, undefined, undefined, { common: item.common }, undefined)
                    : item;
                const isGenesis = block.isGenesis();
                // we cannot overwrite the Genesis block after initializing the Blockchain
                if (isGenesis) {
                    if ((0, ethereumjs_util_1.equalsBytes)(this.genesisBlock.hash(), block.hash())) {
                ...
                const currentTd = { header: ethereumjs_util_1.BIGINT_0, block: ethereumjs_util_1.BIGINT_0 };
                let dbOps = [];
                ...
                if (block.isGenesis() ||
                    td > currentTd.header ||
                    block.common.consensusType() === ethereumjs_common_1.ConsensusType.ProofOfStake) {
                    ...
                    await this._deleteCanonicalChainReferences(blockNumber + ethereumjs_util_1.BIGINT_1, blockHash, dbOps);
    ...
    async validateHeader(header, height) {
        ...
        const { number } = header;
        if (number !== parentHeader.number + ethereumjs_util_1.BIGINT_1) {
        ...
        if (height !== undefined) {
            ...
            if (!(dif < ethereumjs_util_1.BIGINT_8 && dif > ethereumjs_util_1.BIGINT_1)) {
    ...
    async _validateUncleHeaders(block) {
        ...
        const getBlocks = Number(block.header.number - lowestUncleNumber + ethereumjs_util_1.BIGINT_1);
        ...
        for (let i = 0; i < getBlocks; i++) {
            ...
            canonicalChainHashes[(0, ethereumjs_util_1.bytesToUnprefixedHex)(parentBlock.hash())] = true;
            // for each of the uncles, mark the uncle as included
            parentBlock.uncleHeaders.map((uh) => {
                includedUncles[(0, ethereumjs_util_1.bytesToUnprefixedHex)(uh.hash())] = true;
            });
        ...
        uncleHeaders.map((uh) => {
            const uncleHash = (0, ethereumjs_util_1.bytesToUnprefixedHex)(uh.hash());
            const parentHash = (0, ethereumjs_util_1.bytesToUnprefixedHex)(uh.parentHash);
    ...
    async getBlock(blockId) {
        ...
        if (block === undefined) {
            if (typeof blockId === 'object') {
                throw new Error(`Block with hash ${(0, ethereumjs_util_1.bytesToHex)(blockId)} not found in DB`);
            }
            else {
    ...
    async getTotalDifficulty(hash, number) {
        if (number === undefined) {
            ...
            if (number === undefined) {
                throw new Error(`Block with hash ${(0, ethereumjs_util_1.bytesToHex)(hash)} not found in DB`);
            }
        }
    ...
    async getParentTD(header) {
        return header.number === ethereumjs_util_1.BIGINT_0
            ? header.difficulty
            : this.getTotalDifficulty(header.parentHash, header.number - ethereumjs_util_1.BIGINT_1);
    }
    /**
    ...
    async _delBlock(blockHash) {
        ...
        const inCanonical = canonicalHash !== false && (0, ethereumjs_util_1.equalsBytes)(canonicalHash, blockHash);
    ...
    async _delChild(blockHash, blockNumber, headHash, ops) {
        ...
        if (this._headHeaderHash !== undefined &&
            (0, ethereumjs_util_1.equalsBytes)(this._headHeaderHash, blockHash) === true) {
            this._headHeaderHash = headHash;
        }
        if (this._headBlockHash !== undefined && (0, ethereumjs_util_1.equalsBytes)(this._headBlockHash, blockHash)) {
            this._headBlockHash = headHash;
        }
        try {
            const childHeader = await this.getCanonicalHeader(blockNumber + ethereumjs_util_1.BIGINT_1);
    ...
    async iterator(name, onBlock, maxBlocks, releaseLockOnCallback) {
        return this.runWithLock(async () => {
            ...
            let nextBlockNumber = headBlockNumber + ethereumjs_util_1.BIGINT_1;
                ...
                while (maxBlocks !== blocksRanCounter) {
                    try {
                        ...
                        const reorg = lastBlock
                            ? !(0, ethereumjs_util_1.equalsBytes)(lastBlock.hash(), nextBlock.header.parentHash)
                            : false;
                        if (reorg) {
                            ...
                            nextBlockNumber = headBlockNumber + ethereumjs_util_1.BIGINT_1;
                        ...
                        try {
                            ...
                            if (releaseLockOnCallback === true) {
                                ...
                                reorgWhileOnBlock = nextBlockMayBeReorged
                                    ? !(0, ethereumjs_util_1.equalsBytes)(nextBlockMayBeReorged.hash(), nextBlock.hash())
                                    : true;
                            }
                        }
    ...
    async findCommonAncestor(newHeader) {
        ...
        if (header.number > newHeader.number) {
            ...
            while (header.number !== newHeader.number && newHeader.number > ethereumjs_util_1.BIGINT_0) {
                newHeader = await this._getHeader(newHeader.parentHash, newHeader.number - ethereumjs_util_1.BIGINT_1);
        ...
        while (!(0, ethereumjs_util_1.equalsBytes)(header.hash(), newHeader.hash()) && header.number > ethereumjs_util_1.BIGINT_0) {
            header = await this.getCanonicalHeader(header.number - ethereumjs_util_1.BIGINT_1);
            ancestorHeaders.add(header);
            newHeader = await this._getHeader(newHeader.parentHash, newHeader.number - ethereumjs_util_1.BIGINT_1);
            ancestorHeaders.add(newHeader);
        }
        if (!(0, ethereumjs_util_1.equalsBytes)(header.hash(), newHeader.hash())) {
            throw new Error('Failed to find ancient header');
        }
        return {
        ...
        try {
            ...
            while (hash !== false) {
                ...
                for (const name of Object.keys(this._heads)) {
                    if ((0, ethereumjs_util_1.equalsBytes)(this._heads[name], hash)) {
                        this._heads[name] = headHash;
                    }
                }
                // reset stale headHeader to current canonical
                if (this._headHeaderHash !== undefined &&
                    (0, ethereumjs_util_1.equalsBytes)(this._headHeaderHash, hash) === true) {
                    this._headHeaderHash = headHash;
                }
                // reset stale headBlock to current canonical
                if (this._headBlockHash !== undefined && (0, ethereumjs_util_1.equalsBytes)(this._headBlockHash, hash) === true) {
                    this._headBlockHash = headHash;
                }
                blockNumber++;
                hash = await this.safeNumberToHash(blockNumber);
    ...
    async _rebuildCanonical(header, ops) {
        ...
        const loopCondition = async () => {
            ...
            return staleHash === false || !(0, ethereumjs_util_1.equalsBytes)(currentCanonicalHash, staleHash);
        };
        while (await loopCondition()) {
            ...
            for (const name of Object.keys(this._heads)) {
                if (staleHash && (0, ethereumjs_util_1.equalsBytes)(this._heads[name], staleHash)) {
            ...
            if (staleHash &&
                ...
                (0, ethereumjs_util_1.equalsBytes)(this._headBlockHash, staleHash) === true) {
                staleHeadBlock = true;
            }
    ...
    _saveHeadOps() {
        ...
        const hexHeads = Object.fromEntries(Object.entries(this._heads).map((entry) => [entry[0], (0, ethereumjs_util_1.bytesToUnprefixedHex)(entry[1])]));
        return [
    ...
    async _getHeader(hash, number) {
        if (number === undefined) {
            number = await this.dbManager.hashToNumber(hash);
            if (number === undefined)
                throw new Error(`no header for ${(0, ethereumjs_util_1.bytesToHex)(hash)} found in DB`);
        }
    ...
    createGenesisBlock(stateRoot) {
        ...
        if (common.consensusType() === 'poa') {
            if (common.genesis().extraData) {
                ...
                header.extraData = (0, ethereumjs_util_1.concatBytes)(new Uint8Array(32), new Uint8Array(65));
            }
        }
