...
exports.CLIQUE_NONCE_AUTH = (0, ethereumjs_util_1.hexToBytes)('0xffffffffffffffff');
...
exports.CLIQUE_DIFF_INTURN = ethereumjs_util_1.BIGINT_2;
// Block difficulty for out-of-turn signatures
exports.CLIQUE_DIFF_NOTURN = ethereumjs_util_1.BIGINT_1;
/**
...
class CliqueConsensus {
    ...
    async newBlock(block, commonAncestor) {
        ...
        if (commonAncestorNumber !== undefined) {
            await this._cliqueDeleteSnapshots(commonAncestorNumber + ethereumjs_util_1.BIGINT_1);
    ...
    async cliqueUpdateSignerStates(signerState) {
        ...
        const formatted = this._cliqueLatestSignerStates.map((state) => [
            (0, ethereumjs_util_1.bigIntToBytes)(state[0]),
            state[1].map((a) => a.toBytes()),
        ]);
        await this.blockchain.db.put(CLIQUE_SIGNERS_KEY, ethereumjs_rlp_1.RLP.encode(formatted));
    ...
    async cliqueUpdateVotes(header) {
            ...
            for (let round = 1; round <= 2; round++) {
                ...
                let votes = this._cliqueLatestVotes.filter((vote) => {
                    return (vote[0] >= BigInt(lastEpochBlockNumber) &&
                        ...
                        (0, ethereumjs_util_1.equalsBytes)(vote[1][2], exports.CLIQUE_NONCE_AUTH));
                });
                ...
                if (round === 2 && (0, ethereumjs_util_1.equalsBytes)(nonce, exports.CLIQUE_NONCE_AUTH)) {
                ...
                if (numBeneficiaryVotesAUTH >= limit) {
                    ...
                    activeSigners.sort((a, b) => {
                        // Sort by array size
                        const result = (0, ethereumjs_util_1.toType)(a.toString(), ethereumjs_util_1.TypeOutput.BigInt) < (0, ethereumjs_util_1.toType)(b.toString(), ethereumjs_util_1.TypeOutput.BigInt);
                        if (result) {
                            return -1;
                        }
                        else {
                ...
                votes = this._cliqueLatestVotes.filter((vote) => {
                    return (vote[0] >= BigInt(lastEpochBlockNumber) &&
                        !vote[1][0].equals(signer) &&
                        vote[1][1].equals(beneficiary) &&
                        (0, ethereumjs_util_1.equalsBytes)(vote[1][2], exports.CLIQUE_NONCE_DROP));
                });
                ...
                if (round === 2 && (0, ethereumjs_util_1.equalsBytes)(nonce, exports.CLIQUE_NONCE_DROP)) {
                ...
                if (round === 1) {
                    ...
                    debug(`[Block ${header.number}] New clique vote: ${signer} -> ${beneficiary} ${(0, ethereumjs_util_1.equalsBytes)(nonce, exports.CLIQUE_NONCE_AUTH) ? 'AUTH' : 'DROP'}`);
                }
                if (consensus) {
                    if (round === 1) {
        ...
        const formatted = this._cliqueLatestVotes.map((v) => [
            (0, ethereumjs_util_1.bigIntToBytes)(v[0]),
            [v[1][0].toBytes(), v[1][1].toBytes(), v[1][2]],
        ]);
        await this.blockchain.db.put(CLIQUE_VOTES_KEY, ethereumjs_rlp_1.RLP.encode(formatted));
    }
    /**
    ...
    async cliqueUpdateLatestBlockSigners(header) {
        ...
        const formatted = this._cliqueLatestBlockSigners.map((b) => [
            (0, ethereumjs_util_1.bigIntToBytes)(b[0]),
            b[1].toBytes(),
        ]);
        await this.blockchain.db.put(CLIQUE_BLOCK_SIGNERS_SNAPSHOT_KEY, ethereumjs_rlp_1.RLP.encode(formatted));
    }
    /**
    ...
    async getCliqueLatestSignerStates() {
        ...
        if (signerStates === undefined)
            return [];
        const states = ethereumjs_rlp_1.RLP.decode(signerStates);
        return states.map((state) => {
            const blockNum = (0, ethereumjs_util_1.bytesToBigInt)(state[0]);
            const addrs = state[1].map((bytes) => new ethereumjs_util_1.Address(bytes));
    ...
    async getCliqueLatestVotes() {
        ...
        if (signerVotes === undefined)
            return [];
        const votes = ethereumjs_rlp_1.RLP.decode(signerVotes);
        return votes.map((vote) => {
            const blockNum = (0, ethereumjs_util_1.bytesToBigInt)(vote[0]);
    ...
    async getCliqueLatestBlockSigners() {
        ...
        if (blockSigners === undefined)
            return [];
        const signers = ethereumjs_rlp_1.RLP.decode(blockSigners);
        return signers.map((s) => {
            const blockNum = (0, ethereumjs_util_1.bytesToBigInt)(s[0]);
