...
class Interpreter {
    ...
    constructor(evm, stateManager, blockchain, env, gasLeft, journal, performanceLogs, profilerOpts) {
        ...
        this._runState = {
            ...
            memoryWordCount: ethereumjs_util_1.BIGINT_0,
            highestMemCost: ethereumjs_util_1.BIGINT_0,
    ...
    async _runStepHook(dynamicFee, gasLeft) {
        ...
        if (this._evm.DEBUG) {
            ...
            const opTrace = {
                ...
                gasCost: (0, ethereumjs_util_1.bigIntToHex)(dynamicFee),
                stack: hexStack,
        ...
        for (let i = 0; i < code.length; i++) {
            ...
            if (opcode <= 0x7f) {
                if (opcode >= 0x60) {
                    ...
                    const push = (0, ethereumjs_util_1.bytesToBigInt)(code.slice(i + 1, i + opcode - 0x5e));
                    pushes[i + 1] = push;
                    i += extraSteps;
                }
                else if (opcode === 0x5b) {
    ...
    useGas(amount, context) {
        ...
        if (this._runState.gasLeft < ethereumjs_util_1.BIGINT_0) {
            this._runState.gasLeft = ethereumjs_util_1.BIGINT_0;
    ...
    subRefund(amount, context) {
        ...
        this._runState.gasRefund -= amount;
        if (this._runState.gasRefund < ethereumjs_util_1.BIGINT_0) {
            this._runState.gasRefund = ethereumjs_util_1.BIGINT_0;
     ...
     */
    getCaller() {
        return (0, ethereumjs_util_1.bytesToBigInt)(this._env.caller.bytes);
    }
    /**
     ...
     */
    getTxOrigin() {
        return (0, ethereumjs_util_1.bytesToBigInt)(this._env.origin.bytes);
    }
    /**
    ...
    getBlockCoinbase() {
        ...
        return (0, ethereumjs_util_1.bytesToBigInt)(coinbase.toBytes());
    }
    /**
     ...
     */
    getBlockPrevRandao() {
        return (0, ethereumjs_util_1.bytesToBigInt)(this._env.block.header.prevRandao);
    }
    /**
    ...
    async _baseCall(msg) {
        ...
        if (!results.execResult.exceptionError) {
            ...
            this._env.contract = account;
            this._runState.gasRefund = results.execResult.gasRefund ?? ethereumjs_util_1.BIGINT_0;
        }
    ...
    async create(gasLimit, value, data, salt) {
        ...
        this._env.contract.nonce += ethereumjs_util_1.BIGINT_1;
        ...
        if (!results.execResult.exceptionError ||
            ...
            if (results.createdAddress) {
                ...
                return (0, ethereumjs_util_1.bytesToBigInt)(results.createdAddress.bytes);
            }
        }
        return this._getReturnCode(results);
    }
    ...
    async _selfDestruct(toAddress) {
        ...
        if (!this._result.selfdestruct.has((0, ethereumjs_util_1.bytesToHex)(this._env.address.bytes))) {
            this.refundGas(this.common.param('gasPrices', 'selfdestructRefund'));
        }
        this._result.selfdestruct.add((0, ethereumjs_util_1.bytesToHex)(this._env.address.bytes));
        const toSelf = (0, ethereumjs_util_1.equalsBytes)(toAddress.bytes, this._env.address.bytes);
        // Add to beneficiary balance
        if (!toSelf) {
            let toAccount = await this._stateManager.getAccount(toAddress);
            if (!toAccount) {
                toAccount = new ethereumjs_util_1.Account();
            }
        ...
        if (doModify) {
            await this._stateManager.modifyAccountFields(this._env.address, {
                balance: ethereumjs_util_1.BIGINT_0,
            });
        }
        (0, index_js_1.trap)(exceptions_js_1.ERROR.STOP);
    }
    ...
    _getReturnCode(results) {
        if (results.execResult.exceptionError) {
            ...
            return ethereumjs_util_1.BIGINT_1;
        }
    }
}
