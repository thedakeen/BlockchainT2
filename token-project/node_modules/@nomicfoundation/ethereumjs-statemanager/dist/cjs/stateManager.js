 ...
 */
exports.CODEHASH_PREFIX = (0, ethereumjs_util_1.utf8ToBytes)('c');
/**
...
class DefaultStateManager {
    ...
    constructor(opts = {}) {
        ...
        this._proofTrie = new ethereumjs_trie_1.Trie({ useKeyHashing: true, common: this.common });
        this._checkpointCount = 0;
        this._trie = opts.trie ?? new ethereumjs_trie_1.Trie({ useKeyHashing: true, common: this.common });
        this._storageTries = {};
    ...
    async putContractCode(address, value) {
        ...
        if ((0, ethereumjs_util_1.equalsBytes)(codeHash, ethereumjs_util_1.KECCAK256_NULL)) {
            return;
        }
        if (this.DEBUG) {
        ...
        if ((await this.getAccount(address)) === undefined) {
            await this.putAccount(address, new ethereumjs_util_1.Account());
        }
    ...
    async getContractCode(address) {
        ...
        const key = this._prefixCodeHashes
            ? (0, ethereumjs_util_1.concatBytes)(exports.CODEHASH_PREFIX, account.codeHash)
            : account.codeHash;
    ...
    _getStorageTrie(addressOrHash, account) {
        ...
        const addressHex = (0, ethereumjs_util_1.bytesToUnprefixedHex)(addressOrHash instanceof ethereumjs_util_1.Address ? this.keccakFunction(addressOrHash.bytes) : addressOrHash);
        let storageTrie = this._storageTries[addressHex];
        if (storageTrie === undefined) {
            const keyPrefix = this._prefixStorageTrieKeys
                ? (addressOrHash instanceof ethereumjs_util_1.Address
    ...
    async getContractStorage(address, key) {
        ...
        if (!this._storageCacheSettings.deactivate) {
            ...
            if (value !== undefined) {
                const decoded = ethereumjs_rlp_1.RLP.decode(value ?? new Uint8Array(0));
                return decoded;
            }
        }
        const account = await this.getAccount(address);
        ...
        if (!this._storageCacheSettings.deactivate) {
            this._storageCache?.put(address, key, value ?? (0, ethereumjs_util_1.hexToBytes)('0x80'));
        }
        const decoded = ethereumjs_rlp_1.RLP.decode(value ?? new Uint8Array(0));
        return decoded;
    }
    ...
    async _writeContractStorage(address, account, key, value) {
        await this._modifyContractStorage(address, account, async (storageTrie, done) => {
            if (value instanceof Uint8Array && value.length) {
                // format input
                const encodedValue = ethereumjs_rlp_1.RLP.encode(value);
                if (this.DEBUG) {
    ...
    async putContractStorage(address, key, value) {
        ...
        value = (0, ethereumjs_util_1.unpadBytes)(value);
        if (!this._storageCacheSettings.deactivate) {
            const encodedValue = ethereumjs_rlp_1.RLP.encode(value);
    ...
    async flush() {
        if (!this._codeCacheSettings.deactivate) {
            ...
            for (const item of items) {
                const addr = ethereumjs_util_1.Address.fromString(`0x${item[0]}`);
                ...
                const key = this._prefixCodeHashes ? (0, ethereumjs_util_1.concatBytes)(exports.CODEHASH_PREFIX, codeHash) : codeHash;
                await this._getCodeDB().put(key, code);
                // update code root of associated account
                if ((await this.getAccount(addr)) === undefined) {
                    await this.putAccount(addr, new ethereumjs_util_1.Account());
                }
        ...
        if (!this._storageCacheSettings.deactivate) {
            ...
            for (const item of items) {
                const address = ethereumjs_util_1.Address.fromString(`0x${item[0]}`);
                const keyHex = item[1];
                const keyBytes = (0, ethereumjs_util_1.unprefixedHexToBytes)(keyHex);
                const value = item[2];
                const decoded = ethereumjs_rlp_1.RLP.decode(value ?? new Uint8Array(0));
                const account = await this.getAccount(address);
                if (account) {
        ...
        if (!this._accountCacheSettings.deactivate) {
            ...
            for (const item of items) {
                ...
                const addressBytes = (0, ethereumjs_util_1.unprefixedHexToBytes)(addressHex);
                const elem = item[1];
    ...
    async getProof(address, storageSlots = []) {
        ...
        if (!account) {
            ...
            const returnValue = {
                ...
                balance: '0x0',
                codeHash: ethereumjs_util_1.KECCAK256_NULL_S,
                nonce: '0x0',
                storageHash: ethereumjs_util_1.KECCAK256_RLP_S,
                accountProof: (await this._trie.createProof(address.bytes)).map((p) => (0, ethereumjs_util_1.bytesToHex)(p)),
                storageProof: [],
            };
            return returnValue;
        }
        const accountProof = (await this._trie.createProof(address.bytes)).map((p) => (0, ethereumjs_util_1.bytesToHex)(p));
        const storageProof = [];
        const storageTrie = this._getStorageTrie(address, account);
        for (const storageKey of storageSlots) {
            const proof = (await storageTrie.createProof(storageKey)).map((p) => (0, ethereumjs_util_1.bytesToHex)(p));
            const value = (0, ethereumjs_util_1.bytesToHex)(await this.getContractStorage(address, storageKey));
            const proofItem = {
                key: (0, ethereumjs_util_1.bytesToHex)(storageKey),
        ...
        const returnValue = {
            ...
            codeHash: (0, ethereumjs_util_1.bytesToHex)(account.codeHash),
            nonce: (0, ethereumjs_util_1.bigIntToHex)(account.nonce),
            storageHash: (0, ethereumjs_util_1.bytesToHex)(account.storageRoot),
            accountProof,
            storageProof,
        };
        return returnValue;
        ...
        if (Array.isArray(proof)) {
            if (proof.length === 0) {
                ...
                const trie = opts.trie ??
                    (await ethereumjs_trie_1.Trie.createTrieFromProof(proof[0].accountProof.map((e) => (0, ethereumjs_util_1.hexToBytes)(e)), { useKeyHashing: true }));
                const sm = new DefaultStateManager({ ...opts, trie });
                const address = ethereumjs_util_1.Address.fromString(proof[0].address);
    ...
    async addStorageProof(storageProof, storageHash, address, safe = false) {
        ...
        trie.root((0, ethereumjs_util_1.hexToBytes)(storageHash));
        for (let i = 0; i < storageProof.length; i++) {
            await trie.updateTrieFromProof(storageProof[i].proof.map((e) => (0, ethereumjs_util_1.hexToBytes)(e)), safe);
        }
    }
    /**
    ...
    async addProofData(proof, safe = false) {
        if (Array.isArray(proof)) {
            for (let i = 0; i < proof.length; i++) {
                await this._trie.updateTrieFromProof(proof[i].accountProof.map((e) => (0, ethereumjs_util_1.hexToBytes)(e)), safe);
                await this.addStorageProof(proof[i].storageProof, proof[i].storageHash, ethereumjs_util_1.Address.fromString(proof[i].address), safe);
            }
        }
        else {
    ...
    async verifyProof(proof) {
        const rootHash = this.keccakFunction((0, ethereumjs_util_1.hexToBytes)(proof.accountProof[0]));
        const key = (0, ethereumjs_util_1.hexToBytes)(proof.address);
        const accountProof = proof.accountProof.map((rlpString) => (0, ethereumjs_util_1.hexToBytes)(rlpString));
        ...
        if (value === null) {
            ...
            const nonce = (0, ethereumjs_util_1.unpadBytes)((0, ethereumjs_util_1.hexToBytes)(proof.nonce));
            if (!(0, ethereumjs_util_1.equalsBytes)(nonce, emptyBytes)) {
                throw new Error(`${notEmptyErrorMsg} (nonce is not zero)`);
            }
            const balance = (0, ethereumjs_util_1.unpadBytes)((0, ethereumjs_util_1.hexToBytes)(proof.balance));
            if (!(0, ethereumjs_util_1.equalsBytes)(balance, emptyBytes)) {
                throw new Error(`${notEmptyErrorMsg} (balance is not zero)`);
            }
            const storageHash = (0, ethereumjs_util_1.hexToBytes)(proof.storageHash);
            if (!(0, ethereumjs_util_1.equalsBytes)(storageHash, ethereumjs_util_1.KECCAK256_RLP)) {
                throw new Error(`${notEmptyErrorMsg} (storageHash does not equal KECCAK256_RLP)`);
            }
            const codeHash = (0, ethereumjs_util_1.hexToBytes)(proof.codeHash);
            if (!(0, ethereumjs_util_1.equalsBytes)(codeHash, ethereumjs_util_1.KECCAK256_NULL)) {
            ...
            if (!(0, ethereumjs_util_1.equalsBytes)(storageRoot, (0, ethereumjs_util_1.hexToBytes)(proof.storageHash))) {
                throw new Error(`${invalidErrorMsg} storageHash does not match`);
            }
            if (!(0, ethereumjs_util_1.equalsBytes)(codeHash, (0, ethereumjs_util_1.hexToBytes)(proof.codeHash))) {
                throw new Error(`${invalidErrorMsg} codeHash does not match`);
            }
        }
        const storageRoot = (0, ethereumjs_util_1.hexToBytes)(proof.storageHash);
        for (const stProof of proof.storageProof) {
            const storageProof = stProof.proof.map((value) => (0, ethereumjs_util_1.hexToBytes)(value));
            const storageValue = (0, ethereumjs_util_1.setLengthLeft)((0, ethereumjs_util_1.hexToBytes)(stProof.value), 32);
            const storageKey = (0, ethereumjs_util_1.hexToBytes)(stProof.key);
            const proofValue = await this._proofTrie.verifyProof(storageRoot, storageKey, storageProof);
            const reportedValue = (0, ethereumjs_util_1.setLengthLeft)(ethereumjs_rlp_1.RLP.decode(proofValue ?? new Uint8Array(0)), 32);
            if (!(0, ethereumjs_util_1.equalsBytes)(reportedValue, storageValue)) {
    ...
    async setStateRoot(stateRoot, clearCache = true) {
        await this.flush();
        if (!(0, ethereumjs_util_1.equalsBytes)(stateRoot, this._trie.EMPTY_TRIE_ROOT)) {
    ...
    async dumpStorage(address) {
        ...
        for await (const chunk of stream) {
            storage[(0, ethereumjs_util_1.bytesToHex)(chunk.key)] = (0, ethereumjs_util_1.bytesToHex)(chunk.value);
        }
        return storage;
    }
    /**
    ...
    async dumpStorageRange(address, startKey, limit) {
        ...
        for await (const chunk of stream) {
            if (!inRange) {
                ...
                if ((0, ethereumjs_util_1.bytesToBigInt)(chunk.key) >= startKey) {
                    inRange = true;
                }
                else {
                    continue;
            ...
            if (i < limit) {
                storageMap[(0, ethereumjs_util_1.bytesToHex)(chunk.key)] = { key: null, value: (0, ethereumjs_util_1.bytesToHex)(chunk.value) };
                i++;
            }
            else if (i === limit) {
                return {
                    storage: storageMap,
                    nextKey: (0, ethereumjs_util_1.bytesToHex)(chunk.key),
                };
            }
        }
        return {
    ...
    async generateCanonicalGenesis(initState) {
        ...
        for (const address of addresses) {
            ...
            if (!Array.isArray(state)) {
                ...
                const account = ethereumjs_util_1.Account.fromAccountData({ balance, nonce });
                await this.putAccount(addr, account);
                if (code !== undefined) {
                    await this.putContractCode(addr, (0, ethereumjs_util_1.toBytes)(code));
                }
                if (storage !== undefined) {
                    for (const [key, value] of storage) {
                        await this.putContractStorage(addr, (0, ethereumjs_util_1.toBytes)(key), (0, ethereumjs_util_1.toBytes)(value));
                    }
                }
            }
        }
