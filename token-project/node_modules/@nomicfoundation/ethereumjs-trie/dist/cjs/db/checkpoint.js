...
class CheckpointDB {
    ...
    constructor(opts) {
        ...
        this.valueEncoding = opts.valueEncoding ?? ethereumjs_util_1.ValueEncoding.String;
        ...
        if (!this.hasCheckpoints()) {
            ...
            for (const [key, value] of keyValueMap.entries()) {
                if (value === undefined) {
                    batchOp.push({
                        type: 'del',
                        key: (0, ethereumjs_util_1.unprefixedHexToBytes)(key),
                    });
                }
                else {
                    batchOp.push({
    ...
    async get(key) {
        ...
        const returnValue = value !== undefined
            ? value instanceof Uint8Array
                ? value
                : (0, ethereumjs_util_1.unprefixedHexToBytes)(value)
            : undefined;
    ...
    async put(key, value) {
        ...
        if (this.hasCheckpoints()) {
            ...
            const valuePut = this.valueEncoding === ethereumjs_util_1.ValueEncoding.Bytes ? value : (0, ethereumjs_util_1.bytesToUnprefixedHex)(value);
    ...
    async batch(opStack) {
        if (this.hasCheckpoints()) {
            ...
            const convertedOps = opStack.map((op) => {
                const convertedOp = {
                    key: (0, ethereumjs_util_1.bytesToUnprefixedHex)(op.key),
                    value: op.type === 'put' ? op.value : undefined,
                    type: op.type,
                    opts: { ...op.opts, ...{ valueEncoding: this.valueEncoding } },
                };
                if (op.type === 'put' && this.valueEncoding === ethereumjs_util_1.ValueEncoding.String) {
                    convertedOp.value = (0, ethereumjs_util_1.bytesToUnprefixedHex)(convertedOp.value);
                }
                return convertedOp;
            });
