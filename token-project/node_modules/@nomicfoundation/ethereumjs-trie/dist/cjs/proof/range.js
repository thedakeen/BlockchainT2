...
async function unset(trie, parent, child, key, pos, removeLeft, stack) {
    if (child instanceof index_js_1.BranchNode) {
        /**
    ...
    else if (child instanceof index_js_1.ExtensionNode || child instanceof index_js_1.LeafNode) {
        /**
        ...
        if (key.length - pos < child.keyLength() ||
            (0, nibbles_js_1.nibblesCompare)(child._nibbles, key.slice(pos, pos + child.keyLength())) !== 0) {
            if (removeLeft) {
                if ((0, nibbles_js_1.nibblesCompare)(child._nibbles, key.slice(pos)) < 0) {
                    ;
        ...
        if (child instanceof index_js_1.LeafNode) {
            ...
            if (_child instanceof index_js_1.LeafNode) {
...
async function unsetInternal(trie, left, right) {
    ...
    while (true) {
        if (node instanceof index_js_1.ExtensionNode || node instanceof index_js_1.LeafNode) {
            // record this node on the stack
            stack.push(node);
            if (left.length - pos < node.keyLength()) {
                shortForkLeft = (0, nibbles_js_1.nibblesCompare)(left.slice(pos), node._nibbles);
            }
            else {
                shortForkLeft = (0, nibbles_js_1.nibblesCompare)(left.slice(pos, pos + node.keyLength()), node._nibbles);
            }
            if (right.length - pos < node.keyLength()) {
                shortForkRight = (0, nibbles_js_1.nibblesCompare)(right.slice(pos), node._nibbles);
            }
            else {
                shortForkRight = (0, nibbles_js_1.nibblesCompare)(right.slice(pos, pos + node.keyLength()), node._nibbles);
            }
        ...
        else if (node instanceof index_js_1.BranchNode) {
            // record this node on the stack
            stack.push(node);
            ...
            if (!(leftNode instanceof Uint8Array)) {
                ...
                for (let i = 0; i < leftNode.length; i++) {
                    if (!(0, ethereumjs_util_1.equalsBytes)(leftNode[i], rightNode[i])) {
                        abort = true;
                        break;
                    }
                }
                ...
                if (!(0, ethereumjs_util_1.equalsBytes)(leftNode, rightNode)) {
                    break;
                }
            }
            // continue to the next node
...
async function verifyProof(rootHash, key, proof, useKeyHashingFunction) {
    const proofTrie = new trie_js_1.Trie({ root: rootHash, useKeyHashingFunction });
    try {
...
async function hasRightElement(trie, key) {
    ...
    let node = await trie.lookupNode(trie.root());
    while (node !== null) {
        if (node instanceof index_js_1.BranchNode) {
            for (let i = key[pos] + 1; i < 16; i++) {
        ...
        else if (node instanceof index_js_1.ExtensionNode) {
            if (key.length - pos < node.keyLength() ||
                (0, nibbles_js_1.nibblesCompare)(node._nibbles, key.slice(pos, pos + node.keyLength())) !== 0) {
                return (0, nibbles_js_1.nibblesCompare)(node._nibbles, key.slice(pos)) > 0;
            }
            pos += node.keyLength();
            node = await trie.lookupNode(node._value);
        }
        else if (node instanceof index_js_1.LeafNode) {
            return false;
        }
        else {
            throw new Error('invalid node');
...
async function verifyRangeProof(rootHash, firstKey, lastKey, keys, values, proof, useKeyHashingFunction) {
    ...
    for (let i = 0; i < keys.length - 1; i++) {
        if ((0, nibbles_js_1.nibblesCompare)(keys[i], keys[i + 1]) >= 0) {
    ...
    if (proof === null && firstKey === null && lastKey === null) {
        const trie = new trie_js_1.Trie({ useKeyHashingFunction });
        for (let i = 0; i < keys.length; i++) {
            await trie.put((0, nibbles_js_1.nibblestoBytes)(keys[i]), values[i]);
        }
        if (!(0, ethereumjs_util_1.equalsBytes)(rootHash, trie.root())) {
    ...
    // One element proof
    if (keys.length === 1 && (0, nibbles_js_1.nibblesCompare)(firstKey, lastKey) === 0) {
        const { trie, value } = await verifyProof(rootHash, (0, nibbles_js_1.nibblestoBytes)(firstKey), proof, useKeyHashingFunction);
        if ((0, nibbles_js_1.nibblesCompare)(firstKey, keys[0]) !== 0) {
            throw new Error('invalid one element proof: firstKey should be equal to keys[0]');
        }
        if (value === null || !(0, ethereumjs_util_1.equalsBytes)(value, values[0])) {
            throw new Error('invalid one element proof: value mismatch');
        }
        return hasRightElement(trie, firstKey);
    }
    // Two edge elements proof
    if ((0, nibbles_js_1.nibblesCompare)(firstKey, lastKey) >= 0) {
    ...
    const trie = new trie_js_1.Trie({ root: rootHash, useKeyHashingFunction });
    ...
    // Compare rootHash
    if (!(0, ethereumjs_util_1.equalsBytes)(trie.root(), rootHash)) {
