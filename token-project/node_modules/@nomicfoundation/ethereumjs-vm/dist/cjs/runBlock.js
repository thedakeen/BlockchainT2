...
const parentBeaconBlockRootAddress = ethereumjs_util_1.Address.fromString('0x000F3df6D732807Ef1319fB7B8bB8522d0Beac02');
...
async function runBlock(opts) {
    ...
    if (enableProfiler) {
        const title = `Profiler run - Block ${block.header.number} (${(0, ethereumjs_util_1.bytesToHex)(block.hash())} with ${block.transactions.length} txs`;
        // eslint-disable-next-line no-console
        console.log(title);
        // eslint-disable-next-line no-console
    ...
    if (this.DEBUG) {
        debug('-'.repeat(100));
        debug(`Running block hash=${(0, ethereumjs_util_1.bytesToHex)(block.hash())} number=${block.header.number} hardfork=${this.common.hardfork()}`);
    }
    // Set state root if provided
    if (root) {
        if (this.DEBUG) {
            debug(`Set provided state root ${(0, ethereumjs_util_1.bytesToHex)(root)} clearCache=${clearCache}`);
        }
        await state.setStateRoot(root, clearCache);
    }
    if (this.common.isActivatedEIP(6800)) {
        if (!(state instanceof ethereumjs_statemanager_1.StatelessVerkleStateManager)) {
    ...
    if (this.common.hardforkIsActiveOnBlock(ethereumjs_common_1.Hardfork.Dao, block.header.number) === true &&
        block.header.number === this.common.hardforkBlock(ethereumjs_common_1.Hardfork.Dao)) {
        if (this.DEBUG) {
    ...
    try {
        ...
        if (this.DEBUG) {
            debug(`Received block results gasUsed=${result.gasUsed} bloom=${(0, ethereumjs_util_1.short)(result.bloom.bitvector)} (${result.bloom.bitvector.length} bytes) receiptsRoot=${(0, ethereumjs_util_1.bytesToHex)(result.receiptsRoot)} receipts=${result.receipts.length} txResults=${result.results.length}`);
        }
    }
    catch (err) {
    ...
    if (generateFields) {
        ...
        block = ethereumjs_block_1.Block.fromBlockData(blockData, { common: this.common });
    }
    else if (this.common.isActivatedEIP(6800) === false) {
        // Only validate the following headers if verkle blocks aren't activated
        if ((0, ethereumjs_util_1.equalsBytes)(result.receiptsRoot, block.header.receiptTrie) === false) {
            if (this.DEBUG) {
                debug(`Invalid receiptTrie received=${(0, ethereumjs_util_1.bytesToHex)(result.receiptsRoot)} expected=${(0, ethereumjs_util_1.bytesToHex)(block.header.receiptTrie)}`);
            }
            const msg = _errorMsg('invalid receiptTrie', this, block);
            throw new Error(msg);
        }
        if (!((0, ethereumjs_util_1.equalsBytes)(result.bloom.bitvector, block.header.logsBloom) === true)) {
            if (this.DEBUG) {
                debug(`Invalid bloom received=${(0, ethereumjs_util_1.bytesToHex)(result.bloom.bitvector)} expected=${(0, ethereumjs_util_1.bytesToHex)(block.header.logsBloom)}`);
            }
        ...
        if (!((0, ethereumjs_util_1.equalsBytes)(stateRoot, block.header.stateRoot) === true)) {
            if (this.DEBUG) {
                debug(`Invalid stateRoot received=${(0, ethereumjs_util_1.bytesToHex)(stateRoot)} expected=${(0, ethereumjs_util_1.bytesToHex)(block.header.stateRoot)}`);
            }
            const msg = _errorMsg(`invalid block stateRoot, got: ${(0, ethereumjs_util_1.bytesToHex)(stateRoot)}, want: ${(0, ethereumjs_util_1.bytesToHex)(block.header.stateRoot)}`, this, block);
            throw new Error(msg);
        }
    }
    ...
    if (this.DEBUG) {
        debug(`Running block finished hash=${(0, ethereumjs_util_1.bytesToHex)(block.hash())} number=${block.header.number} hardfork=${this.common.hardfork()}`);
    }
    if (enableProfiler) {
        // eslint-disable-next-line no-console
...
async function applyBlock(block, opts) {
    ...
    if (block.common.consensusType() === ethereumjs_common_1.ConsensusType.ProofOfWork) {
...
async function accumulateParentBeaconBlockRoot(root, timestamp) {
    ...
    if ((await this.stateManager.getAccount(parentBeaconBlockRootAddress)) === undefined) {
        await this.evm.journal.putAccount(parentBeaconBlockRootAddress, new ethereumjs_util_1.Account());
    }
    await this.stateManager.putContractStorage(parentBeaconBlockRootAddress, (0, ethereumjs_util_1.setLengthLeft)((0, ethereumjs_util_1.bigIntToBytes)(timestampIndex), 32), (0, ethereumjs_util_1.bigIntToBytes)(timestamp));
    await this.stateManager.putContractStorage(parentBeaconBlockRootAddress, (0, ethereumjs_util_1.setLengthLeft)((0, ethereumjs_util_1.bigIntToBytes)(timestampExtended), 32), root);
}
...
async function applyTransactions(block, opts) {
    ...
    let gasUsed = ethereumjs_util_1.BIGINT_0;
    let receiptTrie = undefined;
    if (block.transactions.length !== 0) {
        receiptTrie = new ethereumjs_trie_1.Trie({ common: this.common });
    }
    const receipts = [];
    const txResults = [];
    /*
    ...
    for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
        ...
        await receiptTrie.put(ethereumjs_rlp_1.RLP.encode(txIdx), encodedReceipt);
    }
    if (enableProfiler) {
        // eslint-disable-next-line no-console
        console.timeEnd(processTxsLabel);
    }
    const receiptsRoot = receiptTrie !== undefined ? receiptTrie.root() : ethereumjs_util_1.KECCAK256_RLP;
    return {
        bloom,
        gasUsed,
        receiptsRoot,
...
async function assignWithdrawals(block) {
    ...
    for (const withdrawal of withdrawals) {
        ...
        await rewardAccount(this.evm, address, amount * ethereumjs_util_1.GWEI_TO_WEI);
    }
}
/**
...
function calculateOmmerReward(ommerBlockNumber, blockNumber, minerReward) {
    ...
    let reward = ((ethereumjs_util_1.BIGINT_8 - heightDiff) * minerReward) / ethereumjs_util_1.BIGINT_8;
    if (reward < ethereumjs_util_1.BIGINT_0) {
        reward = ethereumjs_util_1.BIGINT_0;
    }
    return reward;
}
...
function encodeReceipt(receipt, txType) {
    const encoded = ethereumjs_rlp_1.RLP.encode([
        receipt.stateRoot ??
            (receipt.status === 0 ? Uint8Array.from([]) : (0, ethereumjs_util_1.hexToBytes)('0x01')),
        (0, ethereumjs_util_1.bigIntToBytes)(receipt.cumulativeBlockGasUsed),
        receipt.bitvector,
        receipt.logs,
    ]);
    if (txType === ethereumjs_tx_1.TransactionType.Legacy) {
        return encoded;
    }
    // Serialize receipt according to EIP-2718:
    // `typed-receipt = tx-type || receipt-data`
    return (0, ethereumjs_util_1.concatBytes)((0, ethereumjs_util_1.intToBytes)(txType), encoded);
}
...
async function _applyDAOHardfork(evm) {
    ...
    const DAORefundContractAddress = new ethereumjs_util_1.Address((0, ethereumjs_util_1.unprefixedHexToBytes)(DAORefundContract));
    if ((await state.getAccount(DAORefundContractAddress)) === undefined) {
        await evm.journal.putAccount(DAORefundContractAddress, new ethereumjs_util_1.Account());
    }
    let DAORefundAccount = await state.getAccount(DAORefundContractAddress);
    if (DAORefundAccount === undefined) {
        DAORefundAccount = new ethereumjs_util_1.Account();
    }
    for (const addr of DAOAccountList) {
        // retrieve the account and add it to the DAO's Refund accounts' balance.
        const address = new ethereumjs_util_1.Address((0, ethereumjs_util_1.unprefixedHexToBytes)(addr));
        ...
        account.balance = ethereumjs_util_1.BIGINT_0;
        await evm.journal.putAccount(address, account);
    }
...
async function _genTxTrie(block) {
    ...
    const trie = new ethereumjs_trie_1.Trie({ common: block.common });
    for (const [i, tx] of block.transactions.entries()) {
        await trie.put(ethereumjs_rlp_1.RLP.encode(i), tx.serialize());
    }
    return trie.root();
}
/**
