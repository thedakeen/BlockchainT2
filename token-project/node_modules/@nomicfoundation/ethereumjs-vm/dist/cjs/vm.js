...
class VM {
    ...
    constructor(opts = {}) {
        ...
        if (opts.common) {
            ...
            this.common = new ethereumjs_common_1.Common({ chain: DEFAULT_CHAIN });
        }
        if (opts.stateManager) {
            this.stateManager = opts.stateManager;
        }
        else {
            this.stateManager = new ethereumjs_statemanager_1.DefaultStateManager({ common: this.common });
        }
        this.blockchain = opts.blockchain ?? new ethereumjs_blockchain_1.Blockchain({ common: this.common });
    ...
    async init({ genesisState } = {}) {
        ...
        if (this._opts.activatePrecompiles === true && typeof this._opts.stateManager === 'undefined') {
            ...
            for (const [addressStr] of (0, ethereumjs_evm_1.getActivePrecompiles)(this.common)) {
                const address = new ethereumjs_util_1.Address((0, ethereumjs_util_1.unprefixedHexToBytes)(addressStr));
    ...
    async shallowCopy(downlevelCaches = true) {
        ...
        const evmCopy = new ethereumjs_evm_1.EVM(evmOpts); // TODO fixme (should copy the EVMInterface, not default EVM)
        return VM.create({
            stateManager,
            blockchain: this.blockchain,
            common,
