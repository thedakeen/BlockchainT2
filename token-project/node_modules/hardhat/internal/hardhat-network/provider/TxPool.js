...
function serializeTransaction(tx) {
    const rlpSerialization = (0, ethereumjs_util_1.bytesToHex)(tx.data.serialize());
...
function deserializeTransaction(tx, common) {
    ...
    if (fakeFrom !== undefined) {
        ...
        const serialization = (0, ethereumjs_util_1.toBytes)(rlpSerialization);
        if (tx.get("txType") === 1) {
            data =
        ...
        data = ethereumjs_tx_1.TransactionFactory.fromSerializedData((0, ethereumjs_util_1.toBytes)(rlpSerialization), {
            common,
...
class TxPool {
    ...
    getTransactionByHash(hash) {
        const tx = this._getTransactionsByHash().get((0, ethereumjs_util_1.bytesToHex)(hash));
        if (tx !== undefined) {
    ...
    _addPendingTransaction(tx) {
        ...
        this._setTransactionByHash((0, ethereumjs_util_1.bytesToHex)(tx.hash()), serializedTx);
    }
    ...
    async _validateTransaction(tx, senderAddress, senderNonce) {
        if (this._knownTransaction(tx)) {
            throw new errors_1.InvalidInputError(`Known transaction: ${(0, ethereumjs_util_1.bytesToHex)(tx.hash())}`);
        }
        const txNonce = tx.nonce;
    ...
    _transactionExists(tx, txList) {
        const existingTx = txList?.find((etx) => (0, ethereumjs_util_1.equalsBytes)(this._deserializeTransaction(etx).data.hash(), tx.hash()));
    ...
    _deleteTransactionByHash(hash) {
        this._state = this._state.set("hashToTransaction", this._getTransactionsByHash().delete((0, ethereumjs_util_1.bytesToHex)(hash)));
    }
