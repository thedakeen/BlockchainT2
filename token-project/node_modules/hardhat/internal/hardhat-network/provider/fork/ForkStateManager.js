...
const encodeStorageKey = (address, position) => {
    return `${Buffer.from(address).toString("hex")}${Buffer.from((0, ethereumjs_util_1.unpadBytes)(position)).toString("hex")}`;
};
...
class ForkStateManager {
    ...
    getProof(_address, _storageSlots) {
        throw new Error("Method not implemented.");
    }
    ...
    async initializeGenesisAccounts(genesisAccounts) {
        ...
        for (const ga of genesisAccounts) {
            ...
            const noncePromise = this._jsonRpcClient.getTransactionCount(account.address.toBytes(), this._forkBlockNumber);
    ...
    async getAccount(address) {
        ...
        let nonce = localNonce !== undefined ? (0, ethereumjs_util_1.toBytes)(localNonce) : undefined;
        let balance = localBalance !== undefined ? (0, ethereumjs_util_1.toBytes)(localBalance) : undefined;
        let code = localCode !== undefined ? (0, ethereumjs_util_1.toBytes)(localCode) : undefined;
    ...
    async putContractCode(address, value) {
        ...
        const account = (this._state.get(hexAddress) ?? (0, AccountState_1.makeAccountState)()).set("code", (0, ethereumjs_util_1.bytesToHex)(value));
    ...
    async getContractCode(address) {
        ...
        if (localCode !== undefined) {
            return (0, ethereumjs_util_1.toBytes)(localCode);
        }
        const accountData = await this._jsonRpcClient.getAccountData(address, this._contextBlockNumber);
    ...
    async getContractStorage(address, key) {
        ...
        const localValue = account?.get("storage").get((0, ethereumjs_util_1.bytesToHex)(key));
        if (localValue !== undefined) {
            return (0, ethereumjs_util_1.toBytes)(localValue);
        }
        const slotCleared = localValue === null;
        if (contractStorageCleared || slotCleared) {
            return (0, ethereumjs_util_1.toBytes)([]);
        }
        const remoteValue = await this._jsonRpcClient.getStorageAt(address, (0, ethereumjs_util_1.bytesToBigInt)(key), this._contextBlockNumber);
        return (0, ethereumjs_util_1.unpadBytes)(remoteValue);
    }
    async putContractStorage(address, key, value) {
        ...
        const unpaddedValue = (0, ethereumjs_util_1.unpadBytes)(value);
        const hexAddress = address.toString();
        ...
        if (unpaddedValue.length === 0) {
            ...
            newValue = (0, ethereumjs_util_1.bytesToHex)(unpaddedValue);
        }
        const newStorage = currentStorage.set((0, ethereumjs_util_1.bytesToHex)(key), newValue);
    ...
    async checkpoint() {
        ...
        this._stateCheckpoints.push((0, ethereumjs_util_1.bytesToHex)(stateRoot));
    }
    async commit() {
    ...
    async revert() {
        ...
        await this.setStateRoot((0, ethereumjs_util_1.toBytes)(checkpointedRoot));
    }
    async getStateRoot() {
        ...
        return (0, ethereumjs_util_1.toBytes)(this._stateRoot);
    }
    ...
    async accountIsEmpty(address) {
        ...
        return (account.nonce === 0n &&
            account.balance === 0n &&
            (0, ethereumjs_util_1.equalsBytes)(account.codeHash, ethereumjs_util_1.KECCAK256_NULL));
    }
    ...
    setBlockContext(stateRoot, blockNumber, irregularState) {
        ...
        if (blockNumber === this._forkBlockNumber) {
            this._setStateRoot((0, ethereumjs_util_1.toBytes)(this._initialStateRoot));
            return;
        }
        if (blockNumber > this._forkBlockNumber) {
            this._setStateRoot(stateRoot);
        ...
        this._state = (0, immutable_1.Map)();
        this._stateRoot = (0, ethereumjs_util_1.bytesToHex)(stateRoot);
        this._stateRootToState.set(this._stateRoot, this._state);
    ...
    _putAccount(address, account) {
        ...
        if ((0, ethereumjs_util_1.equalsBytes)(account.codeHash, ethereumjs_util_1.KECCAK256_NULL)) {
            localAccount = localAccount.set("code", "0x");
        }
        this._state = this._state.set(hexAddress, localAccount);
    }
    _setStateRoot(stateRoot) {
        const newRoot = (0, ethereumjs_util_1.bytesToHex)(stateRoot);
    ...
    async hasStateRoot(root) {
        return this._state.has((0, ethereumjs_util_1.bytesToHex)(root));
    }
    async flush() {
        // not implemented
    }
