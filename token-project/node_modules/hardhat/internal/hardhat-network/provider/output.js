...
function getRpcBlock(block, totalDifficulty, showTransactionType, includeTransactions = true, pending = false) {
    ...
    const output = {
        ...
        miner: (0, base_types_1.bufferToRpcData)(block.header.coinbase.toBytes()),
    ...
    if (block.header.withdrawalsRoot !== undefined) {
        output.withdrawals = block.withdrawals?.map((withdrawal) => ({
            ...
            address: (0, base_types_1.bufferToRpcData)(withdrawal.address.toBytes()),
...
function addCancunPropertiesIfPresent(output, block, pending) {
    if (block.header.parentBeaconBlockRoot !== undefined) {
        output.parentBeaconBlockRoot = pending
            ? null
            : (0, base_types_1.bufferToRpcData)(block.header.parentBeaconBlockRoot, 32);
    }
    if (block.header.blobGasUsed !== undefined) {
        output.blobGasUsed = (0, base_types_1.numberToRpcQuantity)(block.header.blobGasUsed);
    }
    if (block.header.excessBlobGas !== undefined) {
        output.excessBlobGas = (0, base_types_1.numberToRpcQuantity)(block.header.excessBlobGas);
    }
}
function getRpcTransaction(tx, showTransactionType, block, index) {
    ...
    const baseOutput = {
        ...
        from: (0, base_types_1.bufferToRpcData)(tx.getSenderAddress().toBytes()),
        ...
        to: tx.to === undefined ? null : (0, base_types_1.bufferToRpcData)(tx.to.toBytes()),
        ...
        accessList: "accessList" in tx
            ? tx.accessList.map(([address, storageKeys]) => ({
                address: (0, ethereumjs_util_1.bytesToHex)(address),
                storageKeys: storageKeys.map(ethereumjs_util_1.bytesToHex),
            }))
            : undefined,
    ...
    for (let i = 0; i < runBlockResult.results.length; i += 1) {
        ...
        const rpcReceipt = {
            ...
            contractAddress: createdAddress !== undefined
                ? (0, base_types_1.bufferToRpcData)(createdAddress.toBytes())
                : null,
            logs,
