...
class FakeSenderEIP1559Transaction extends ethereumjs_tx_1.FeeMarketEIP1559Transaction {
    ...
    static fromSenderAndValuesArray(sender, values, opts = {}) {
        ...
        return new FakeSenderEIP1559Transaction(sender, {
            ...
            v: v !== undefined ? (0, ethereumjs_util_1.bytesToInt)(v) : undefined,
            r: r !== undefined && r.length !== 0 ? (0, ethereumjs_util_1.bytesToInt)(r) : undefined,
            s: s !== undefined && s.length !== 0 ? (0, ethereumjs_util_1.bytesToInt)(s) : undefined,
        }, opts);
    }
...
function checkIsFeeMarketEIP1559ValuesArray(values) {
    if (!Array.isArray(values)) {
        throw new errors_1.InvalidArgumentsError(`Invalid deserialized tx. Expected a Uint8Array[], but got '${values}'`);
    }
    ...
    for (const [i, value] of values.entries()) {
        if (i === 8) {
            ...
            if (!(values[i] instanceof Uint8Array)) {
                throw new errors_1.InvalidArgumentsError(`Invalid deserialized tx. Expected a Uint8Array in position ${i}, but got '${value}'`);
            }
        }
    }
}
